...
<body>
        ...
        <div class="form-wrapper">
            ...
            <div class="format-helper">
                <table>
                    ...
                    <tr>
                        <td>==> <ion-icon name='arrow-forward'></ion-icon>
                        </td>
                        <td class="special-text-0 test">
    ...
    function getFormData() {
        ...
        for (const rule of ruleSections) {
            ...
            if (example && example.value != "") {
                ...
                textStr +=
                    `<div class="rule-example"><p class="rule-example-header">Παράδειγμα</p><p class="example">${filterText(example.value)}</p></div>`;
            }
            textStr += '</div></div>';
        }
    ...
    function exportHTML() {
        ...
        container.innerHTML = `${String(arr).replace("==>", "<ion-icon name='arrow-forward'></ion-icon>").replace(/&/g, "&amp;")
         .replace(/</g, "&lt;")
         .replace(/>/g, "&gt;")
         .replace(/"/g, "\\&quot;")
         .replace(/'/g, "&#039;")}`;
